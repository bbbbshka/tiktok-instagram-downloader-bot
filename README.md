...
- 🎬 **Скачивание видео** — отправьте ссылку TikTok, Instagram или YouTube → получите видео в чат
- 🔗 **Inline-режим** — напишите `@имя_бота <ссылка>` в любом чате; для надёжного TikTok inline можно включить Telegram cache chat
- ⚡ **Быстрая загрузка** — параллельное скачивание фрагментов, кеширование file_id, максимальное качество
- 🖼️ **Фото-карусели** — TikTok фото-посты отправляются альбомом (gallery-dl)
- 🎵 **Музыка** — кнопка для скачивания аудио-трека из видео
...
| YouTube | `youtube.com/watch`, `youtube.com/shorts`, `youtu.be`, `youtube.com/live` |
...
### 6. Надёжный inline для TikTok (опционально)
Если хотите, чтобы inline-режим работал как у крупных downloader-ботов, создайте скрытый канал/чат, добавьте туда бота админом и укажите его ID в `.env`:

```env
INLINE_CACHE_CHAT_ID=-1001234567890
```

Тогда бот сможет сначала загружать видео в Telegram-кеш и потом отдавать его через inline как `file_id`.

## Оптимизация скорости

Бот оптимизирован для быстрой загрузки видео:

- **Параллельные фрагменты** — yt-dlp скачивает до 16 фрагментов одновременно (`CONCURRENT_FRAGMENTS`)
- **Максимальное качество** — всегда скачивает лучшее доступное разрешение
...
| `CONCURRENT_FRAGMENTS` | нет | Параллельные загрузки фрагментов (по умолчанию 16) |
| `CACHE_TTL_SECONDS` | нет | Время жизни кеша file_id (по умолчанию 3600 сек) |
| `INLINE_CACHE_CHAT_ID` | нет | ID скрытого Telegram чата/канала, куда бот складывает видео для надёжного inline |
